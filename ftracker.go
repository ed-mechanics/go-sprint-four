...
func WalkingSpentCalories(action int, duration, weight, height float64) float64 {
	...
	speedMetersPerSecond := speed * kmhInMsec // Преобразуем в м/с
	return ((walkingCaloriesWeightMultiplier*weight + (math.Pow(speedMetersPerSecond, 2)/(height/cmInM))*walkingSpeedHeightMultiplier*weight) * duration * minInH)
}
